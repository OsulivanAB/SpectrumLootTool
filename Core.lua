...
function SLH:Init()
	...
	if self.db.settings.allowOutsideRaid == nil then
		self.db.settings.allowOutsideRaid = false
	end
	...
	if self.Debug then
		...
		self.Debug:LogInfo("Core", "Debug system fully implemented with performance optimization", {
			...
			optimization = {
				"/slh debuglog optimize run",
				"/slh debuglog optimize memory",
				"/slh debuglog optimize monitor",
			},
...
function SLH:IsOfficer(unit)
	...
	if self.debugOfficer then
		print(
			string.format(
				"|cff00ff00SLH Debug: %s - Guild: '%s', Rank: %d, IsSpectrum: %s, IsOfficer: %s|r",
				unit,
				guild,
				rankIndex,
				tostring(isSpectrumFed),
				tostring(isOfficer)
			)
		)
	end
		...
		if SLH.Log then
			...
			if result.success then
				if result.changed then
					print(
						"|cff00ff00SLH: WoW version changed from "
							.. (result.previousVersion or "none")
							.. " to "
							.. result.newVersion
							.. "|r"
					)
